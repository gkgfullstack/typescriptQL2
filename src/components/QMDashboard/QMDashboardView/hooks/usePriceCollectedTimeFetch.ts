...
import { PriceCollectedTimeResponse, getPriceCollectedTime } from 'src/api/PriceCollectedTime';
...
const initialState: FetchState<PriceCollectedTime> = {
  loading: false,
  error: false,
  data: null,
};

const usePricecollectedTimeFetch = (): [FetchState<PriceCollectedTime>] => {
  ...
  useEffect(() => {
    ...
    const fetch = async () => {
      ...
      try {
        const response: PriceCollectedTimeResponse = await getPriceCollectedTime(sourceOwnerId, initialMatchTypeFilterIds, initialRegionIds, initialPriceTypeIds);
        if (!ignore) {
          dispatch({
            type: FETCH_ACTION_TYPES.setData,
            payload: { data: response.priceCollectedTime},
          });
        }
      } catch (e) {
        if (!ignore) {
  ...
  return [state as FetchState<PriceCollectedTime>];
};

export default usePricecollectedTimeFetch;
