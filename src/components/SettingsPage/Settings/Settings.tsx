...
import { useSettingsPostFetch } from '../hooks';
...
const Settings: React.FC<SupportFormProps> = () => {
  ...
  const [{ data, loading, error }, { settingsUpdatedsss }]: any = useSettingsPostFetch();
  console.log(data, loading, error);
  const addUpdateNameSetting = (values: any): void => {
    if (settingsUpdatedsss && values) {
      settingsUpdatedsss(values);
    }
  };
    ...
    <div className={styles.product_summary}>
      ...
      <TabPane tab={<><FontAwesomeIcon icon={faCogs} /><span>Settings</span> </>} key="/settings/account-settings"></TabPane>
        ...
        <span className={styles.actions_container}>
          <SettingsForm onUpdate={addUpdateNameSetting} />
        </span>
      </div>
    </div>
  );
