...
const statusRender = (record: any, onAction: any) => {
  return (
    <p>
      {record.ownerClientLookup && record.ownerClientLookup.outlierDetectionInclusionFlag ? (
        <FontAwesomeIcon
          icon={faToggleOnIcon}
...
export const getColumns = (sorting: any, onChangeStatus: any, onRegionView: any) => {
  const columns: ColumnConfig[] = [
    ...
    {
      title: 'Outlier Enabled',
      key: 'outlierdetectioninclusionflag',
      defaultSortOrder: 'ascend',
      sortDirections: ['ascend', 'descend', 'ascend'],
      render: record => statusRender(record, onChangeStatus),
      sortOrder: sortOrder(sorting, 'outlierdetectioninclusionflag'),
      sorter: true,
      width: 120,
    },
  ];
