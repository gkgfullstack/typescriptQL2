...
const usePreviewrFetch = (
  ...
  }, [resultid, initialSorting, initialSize, initialPage]);
  useEffect(() => {
    const fetch = async () => {
      if (resultid) {
        ...
        try {
          const { lineItems: data, TotalRecord, columns:columnsg, message, listdata }: RTLPreviewResponse = await getRTLPreview(
            ...
            console.log('usePreviewrFetch.tsx=============', message)
          dispatch({
            type: PREVIEW_ACTION_TYPES.setPreview,
            payload: { data: data, columnsg, total: Number(TotalRecord), message, listdata },
            
          });
          console.log('totalRecords', message)
          console.log('totalRecords', TotalRecord)
          console.log('columns=====', columnsg)
        } catch (e) {
