...
import { useTableauReportFetch } from './hooks';
...
const Insights: React.FC<InsightsProps> = () => {
  ...
  const [{ data: reportUrl, loading, error }] = useTableauReportFetch();
  ...
  return (
    <Widget>
      {error && (
        <Alert
          message="Error"
          description="An error has occurred when trying to get report! Please try again later!"
          type="error"
          showIcon
        />
      )}
      ...
      {reportUrl !== null && <TableauReport url={reportUrl} />}
    </Widget>
  );
};
